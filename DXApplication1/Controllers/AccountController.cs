...
namespace DXApplication1.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            // Eğer giriş yapılmışsa => ÇIKIŞ YAP ve tekrar giriş ekranına yönlendir
            if (User.Identity.IsAuthenticated)
            {
                // Cookie ve oturumu temizle
                AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
                Session.Abandon();

                // Şu anki tarayıcıdaki cookie’yi de bitir
                var cookie = new HttpCookie("ASP.NET.ApplicationCookie");
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    // returnUrl'yi dikkate almadan direkt Anasayfa'ya yönlendir
